...
## GOAL: Given a message block M (I use urandom), find a message block M', differing only in a few bits, 
##       that will collide with it in hash value generated by MD4. Just so long as a short set of conditions holds true for M.
##       Conditions? https://fortenf.org/e/assets/md4/constraints.png

## WHAT TO DO? (main steps)
##      - Enforce those conditions for 3 rounds of MD4 compression function
##      - In each round, iterate over each word in the message block sequentially and mix it into the state.
##        (careful not to stomp on any of the previous round's conditions)
...
def p55():
    ...
    while not collision:
        ...
        # generate M' by flipping a few bits and test for a collision
        m_ = m[:]
        ...
        collision = _md4_hash(M) == _md4_hash(M_)#MD4(M) == MD4(m_)
